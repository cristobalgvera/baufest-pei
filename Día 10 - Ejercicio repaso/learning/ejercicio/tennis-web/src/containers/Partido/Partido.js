...
const Partido = props => {
    ...
    const getCanchas = async () => {
        try {
            ...
            setListaCanchas(await response);
        } catch (e) {
    ...
    const handleFormSubmit = async (form, isEdit) => {
        ...
        console.log(newPartidoData.cancha, newPartidoData.fechaComienzo)
        if (!await validateCanchaOccupancy(newPartidoData.cancha, newPartidoData.fechaComienzo)) return;
    ...
    const validateCanchaOccupancy = async (id, date) => {

        if (!await API.save(`/canchas/${id}/actions/validar-horario`, null, {date})) {
            setErrorMsg('El horario ingresado no es válido, ya que existe otro partido dentro del rango de cuatro horas');
            return false;
        }
