...
const Jugador = props => {
    ...
    const calculateAllRankings = async () => {
        try {
            await API.update('/jugadores/actions/calcular-ranking-todos')
            await getJugadores();
        } catch (e) {
            console.log(e)
        }
    ...
    const calculateRanking = async (id) => {
        try {
            await API.update(`/jugadores/${id}/actions/calcular-ranking`);
            await getJugadores();
        } catch (e) {
            console.log(e);
        }
    ...
    return (
        <div className="container mt-4">
            ...
            <Button variant={"dark mb-3"} onClick={() => calculateAllRankings()}> Calcular ranking </Button>
            <JugadorModal
                show={showModal}
