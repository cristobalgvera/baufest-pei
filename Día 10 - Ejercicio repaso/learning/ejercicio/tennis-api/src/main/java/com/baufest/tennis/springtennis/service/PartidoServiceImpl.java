...
public class PartidoServiceImpl implements PartidoService {
    ...
    public static final String INVALID_DATE_BY_HOURS_IN_PARTIDO = "La hora de inicio debe ser luego de cuatro horas de inicio o término del partido con id = ";

    private final PartidoRepository partidoRepository;
    private final PartidoMapper partidoMapper;
    private final CanchaServiceImpl canchaServiceImpl;
    ...
    @Autowired
    public PartidoServiceImpl(PartidoRepository partidoRepository, PartidoMapper partidoMapper, CanchaServiceImpl canchaServiceImpl) {
        this.partidoRepository = partidoRepository;
        this.partidoMapper = partidoMapper;
        this.canchaServiceImpl = canchaServiceImpl;
    }
    ...
    public List<PartidoDTO> listAll() {
        return partidoRepository.findAll()
                .stream()
                .map(partido -> partidoMapper.toDTO(partido, new CycleAvoidingMappingContext()))
                .collect(Collectors.toList());
    }

    @Override
    public PartidoDTO getById(Long id) {
        return partidoRepository.findById(id).map(partido -> partidoMapper.toDTO(partido, new CycleAvoidingMappingContext()))
    ...
    private void validarFechaYHora(PartidoDTO partido) {
        ...
        validateCanchaOccupancy(partido);
    }

    private void validateCanchaOccupancy(PartidoDTO partido) {
        CanchaDTO canchaDTO = canchaServiceImpl.findById(partido.getCancha().getId());
        validateOccupancy(canchaDTO, partido.getFechaComienzo());
    }

    public boolean validateCanchaOccupancy(Long canchaId, Date date) {
        CanchaDTO canchaDTO = canchaServiceImpl.findById(canchaId);
        return validateOccupancy(canchaDTO, date);
    }

    private boolean validateOccupancy(CanchaDTO canchaDTO, Date partidoDate) {
        Calendar calendar = Calendar.getInstance();

        // I don't know how to properly works with Date. Are LocalDate and LocalDateTime actual Date formats.
        for (PartidoDTO p : canchaDTO.getPartidos()) {
            calendar.setTime(p.getFechaComienzo());

            // Partido date plus four hours
            calendar.add(Calendar.HOUR, 4);
            Date pDatePlusFour = calendar.getTime();

            // Partido date minus four hours
            calendar.add(Calendar.HOUR, -8);
            Date pDateMinusFour = calendar.getTime();

            boolean dateItsWrong = partidoDate.after(pDateMinusFour) && partidoDate.before(pDatePlusFour);

            if (dateItsWrong)
                throw new IllegalArgumentException(INVALID_DATE_BY_HOURS_IN_PARTIDO + p.getId());
        }
    ...
    public PartidoDTO save(PartidoDTO partido) {
        ...
        return this.partidoMapper.toDTO(partidoRepository.save(this.partidoMapper.fromDTO(partido, new CycleAvoidingMappingContext())), new CycleAvoidingMappingContext());
    }
    ...
    public PartidoDTO sumarPuntos(Long id, ModoJugador modo) {
        ...
        if (optPartido.isPresent()) {
            ...
            return this.partidoMapper.toDTO(partidoRepository.save(partido), new CycleAvoidingMappingContext());
        } else {
            throw new NoSuchElementException(PARTIDO_WITH_ID + id + DOES_NOT_EXIST);
        }
    }
