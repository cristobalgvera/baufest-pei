...
import com.baufest.tennis.springtennis.service.util.CanchaServiceMethods;
...
@Service
public class CanchaServiceImpl implements CrudMethods<CanchaDTO, Long>, CanchaServiceMethods {
    ...
    private void validateSaveCancha(CanchaDTO canchaDTO) {
        ...
        boolean hasId = canchaDTO.getId() != null && canchaRepository.existsById(canchaDTO.getId());
    ...
    @Override
    public Collection<PartidoDTO> findAllTodayPartidosByCanchaId(Long id) {
        CanchaDTO canchaDTO = findById(id);
        return canchaDTO.getPartidos()
                .stream()
                .filter(partidoDTO -> DateUtils.isSameDay(new Date(), partidoDTO.getFechaComienzo()))
                .collect(Collectors.toList());
    }
}
